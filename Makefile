...
# QUBES_BRANCH is the prodution branch name in the qubes repo
# FEDORA_DIST is as specified in `distributions` section of builderv2 .yml
QUBES_RELEASE ?= "4.2"
QUBES_BRANCH = "release4.2"
...
build-rpm: $(if $(REPROTEST),qubes-builder) prepare ## Build rpm (default: prod)
	...
	FEDORA_DIST=$(FEDORA_DIST) sd-qubes-builder/build-rpm.sh
...
reprotest: ## Test reproducibility
	...
	@sudo reprotest 'REPROTEST=1 make build-rpm BRANCH="${BRANCH}" EXECUTOR="${EXECUTOR}" EXECUTOROPTS="${EXECUTOROPTS}" QUBES_RELEASE="${QUBES_RELEASE}" QUBES_BRANCH="${QUBES_BRANCH}" FEDORA_DIST="${FEDORA_DIST}"' 'build/*.rpm' --variations '+all,+kernel,-time,-fileordering,-domain_host'
